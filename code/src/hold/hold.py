...
from src.hold.hold_net import HOLDNet, SplatNet
...
class HOLD(pl.LightningModule):
    ...
    def on_train_epoch_end(self) -> None:
        ...
        # return super().training_epoch_end(self.training_step_outputs)
    ...
    def inference_step(self, batch, *args, **kwargs):
        ...
        output.update(batch)
        self.inference_step_outputs.append(output)
        return output
    ...
    def on_validation_epoch_end(self) -> None:
        outputs = self.inference_step_outputs
        if not self.args.no_vis:
            img_size = outputs[0]["img_size"]
            idx = outputs[0]["idx"]
            vis_dict = vis_utils.output2images(outputs, img_size)
            vis_utils.record_vis(
                idx, self.global_step, self.args.log_dir, vis_dict
            )
        self.inference_step_outputs.clear()
...
class HOLDSplat(pl.LightningModule):
    def __init__(self, opt, args) -> None:
        super().__init__()
        self.training_step_outputs = []
        self.inference_step_outputs = []
